...
// 2. Data Array Isolation: Pulled outside component loop to eliminate compiler overhead
...
export default function PropertyGrid({ activeCategory = "All Types" }: PropertyGridProps) {
  ...
  const filteredProperties = properties.filter((property) => {
    ...
    // Normalize string constraints to safeguard real estate verticals layout mappings
    return property.category.toLowerCase() === activeCategory.toLowerCase();
  });

  return (
    <section id="properties" className="max-w-[1400px] mx-auto px-6 sm:px-8 py-16 bg-[#FAF9F6]">
      
      {/* Editorial Header Block */}
      <div className="flex flex-col items-start md:items-center text-left md:text-center mb-20 space-y-4">
        <p className="uppercase tracking-[0.25em] text-[#6B7280] text-xs font-bold">
          Curated Portfolios [cite: 43]
        </p>
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-[#0B1F4D]">
          Explore Premium Properties
        </h2>
        <p className="text-sm text-[#6B7280] max-w-2xl font-normal leading-relaxed">
          Carefully selected villas, premium plots, commercial spaces, and high-yield investment structures across key Himalayan hubs[cite: 13, 14, 15, 17, 19].
        </p>
      </div>
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {filteredProperties.map((property) => (
          <div
            key={property.id}
            className="group flex flex-col rounded-xl overflow-hidden bg-[#FFFFFF] border border-[#E5E7EB]/70 transition-all duration-500 ease-out hover:shadow-xl hover:shadow-[#0B1F4D]/5 transform-gpu"
          >
            
            {/* Cinematic Image Frame [cite: 188] */}
            <div className="relative aspect-[16/10] overflow-hidden bg-[#F5F7FA]">
              ...
              <span className="absolute top-4 left-4 bg-[#0B1F4D] text-white text-[10px] uppercase tracking-widest font-bold px-3 py-1.5 rounded-md shadow-md">
                {property.status}
              </span>
            </div>

            {/* Asset Details Wrapper [cite: 162] */}
            <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between">
              <div>
                <p className="text-[#6B7280] uppercase tracking-[0.2em] text-[10px] font-bold mb-1.5">
                  {property.category}
                </p>
                ...
                <div className="flex items-center gap-1.5 mt-2 text-[#6B7280] text-xs font-medium">
                  ...
                  <span>{property.location}, Uttarakhand [cite: 19]</span>
                </div>

                {/* Specification Grid with fixed-height constraint to block structural layout shifts */}
                <div className="flex items-center gap-6 mt-6 pt-4 border-t border-[#F5F7FA] text-[#6B7280] text-xs font-semibold min-h-[44px]">
                  {property.bedrooms ? (
                    ...
                    // Aesthetic filler for premium layouts missing structural dimensions (e.g., plots)
                    <div className="flex items-center gap-2">
                      <span className="text-gray-300 font-light italic">Premium Tier Land Asset</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2">
              ...
              {/* Pricing Call To Action [cite: 142] */}
              <div className="flex items-end justify-between mt-8 pt-4 border-t border-[#F5F7FA]">
                ...
                {/* Subdued Premium Action Button [cite: 143, 144] */}
                <Link
                  href={`/properties/${property.id}`}
                  className="group/btn flex h-11 w-11 items-center justify-center rounded-xl bg-[#0B1F4D] text-white transition-all duration-300 hover:bg-[#163172] hover:scale-105 transform-gpu"
                >
